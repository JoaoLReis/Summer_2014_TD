...
{ 
	...
	public List<int> hashes = new List<int>(10000);
	public List<Transform> prefabs = new List<Transform>(1000);
	public List<int> directions = new List<int>(10000);
    public List<Transform> instances = new List<Transform>(10000);

    enum TILE_STATE {DEFAULT, BUILD};
    Material def, build;
    GameState game;

	void Start()
	{
        def = Resources.Load<Material>("Materials/TileAreaMaterials/TransparentArea");
        build = Resources.Load<Material>("Materials/TileAreaMaterials/BuildableArea");
    ...
    void Update()
    {
        ...
        {
            if (game.get_BM())
                updateTiles(TILE_STATE.BUILD);
            else updateTiles(TILE_STATE.DEFAULT);
            game.setTileUpdate(false);
        }
    }
